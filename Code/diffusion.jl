...
__precompile__(false)
    ...
    function update_GPU_low_res!(c_old, c_new, N, dtdx2, D, Db, spore_vol_idx, c_spore, tau)
        """
        Update the concentration values on the lattice
        using the time-dependent diffusion equation.
        inputs:
            ...
            c_spore (float) - the concentration at the spore
            tau (float) - characteristic time for permeation
        """
        i, j, k = CUDA.blockIdx().x, CUDA.blockIdx().y, CUDA.blockIdx().z
        ti, tj, tk = CUDA.threadIdx().x, CUDA.threadIdx().y, CUDA.threadIdx().z
        ...
        if 1 ≤ idx[1] ≤ N && 1 ≤ idx[2] ≤ N && 1 ≤ idx[3] ≤ N
            ...
            if idx[1] == spore_vol_idx[1] && idx[2] == spore_vol_idx[2] && idx[3] == spore_vol_idx[3]
                add_c = center .- (center .- c_spore) .* exp.(-t / tau)
            end
        end
    ...
    function diffusion_time_dependent_GPU(c_init, t_max; D=1.0, Db=nothing, Ps=1.0, dt=0.005, dx=5, n_save_frames=100,
        spore_idx=nothing, spore_idx_spacing=nothing, c_thresholds=nothing)
        """
    ...
    function diffusion_time_dependent_GPU_low_res(c_init, t_max; D=1.0, Db=nothing, Ps=1.0, A=150, V=125, dt=150, dx=25, n_save_frames=100,
        spore_vol_idx=nothing, spore_vol_idx_spacing=nothing, c_thresholds=nothing)
        """
        Compute the evolution of a square lattice of concentration scalars
        ...
        for t in 1:n_frames
            ...
            # Update the lattice
            @cuda threads=kernel_threads blocks=kernel_blocks update_GPU_low_res!(c_A_gpu, c_B_gpu, N, dtdx2, D, Db, spore_vol_idx_ref)
            c_A_gpu, c_B_gpu = c_B_gpu, c_A_gpu
